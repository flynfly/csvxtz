...
class PlottingManagement():
            ...
            if len(self.emg_plot) >= 2:
                ...
                if self.dataStreamIdx and self.outData[0].size > 0:
                    try:
                        ...
                        self.emg_data_storage.append([self.emg_plot[0][i][0] for i in self.dataStreamIdx])
                    except IndexError:
    ...
    def Save_Callback(self):
        """Callback to save the data stream"""
        print("save")
        emg_data = np.array(self.emg_data_storage)
        save_path, _ = QFileDialog.getSaveFileName(None, "保存%d个通道数据"%self.SensorsFound, "ddd", "Numpy files (*.npy)")
        # 如果用户选择了路径，则保存数据为npy文件
        if save_path:
            np.save(save_path, emg_data)
